...
function topHotspotsFor(id){
 ...
 if(EXPERIENCE.has(id)||TEMPERATURE_FLOW.has(id))return [hs('Top Alerts',.84,.085,.11,.08,()=>go('alerts'))];
...
[['scorecard',.085,.405],['composite',.355,.405],['maximo',.625,.405],['temperature-1',.085,.528],['sales',.355,.528],['labor-health',.625,.528]].forEach(([id,x,y])=>a.push(hs(titleOf(id),x,y,.27,.12,()=>go(id))));
...
function experienceHotspots(){
 const map={
  'temperature-1':hs('Start Guided Mission',.085,.495,.83,.065,()=>go('temperature-2')),
  'temperature-2':hs('Continue',.07,.792,.86,.065,()=>go('temperature-3')),
  'temperature-3':hs('Continue',.07,.842,.86,.06,()=>go('home')),
